...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute) {
    ...
    final RecordIterator iterator = table.iterator();
    float min = Float.POSITIVE_INFINITY, max = Float.NEGATIVE_INFINITY;

    while (iterator.hasNext()) {
      final Record next = iterator.next();
      final float aFloat = quantization(next, attribute);
      if (aFloat > max) {
        max = aFloat;
      }
      if (aFloat < min) {
    ...
    final float width = (max - min) / numBuckets;

    //3. create each bucket object
    for (int i = 0; i < numBuckets; i++) {
      buckets[i] = new Bucket<>(min + i * width, min + (i + 1) * width);
    }

    //4. populate the data using the increment(value) method
    final RecordIterator iterator1 = table.iterator();
    while (iterator1.hasNext()) {
      final Record next = iterator1.next();
      final float value = quantization(next, attribute);
      if (value == max) {
        buckets[numBuckets - 1].increment(value);
      } else {
  ...
  private float[] allEquality(float qvalue) {
    ...
    for (i = 0; i < buckets.length; i++) {
      final Bucket<Float> bucket = buckets[i];
      if(bucket.getStart() <= qvalue && qvalue < bucket.getEnd()){
        result[i] = (float) (1.0 / bucket.getDistinctCount());
        break;
      }
    }
    if(i == buckets.length && buckets[buckets.length - 1].getEnd() == qvalue){
      result[i-1] = (float)(1.0 / buckets[buckets.length -1].getDistinctCount());
    }
    return result;
  }
  ...
  private float[] allNotEquality(float qvalue) {
    float[] result = new float[this.numBuckets];
    final float[] floats = allEquality(qvalue);
    for (int i = 0; i < numBuckets; i++) {
      result[i] = 1 - floats[i];
    }
  ...
  private float[] allGreaterThan(float qvalue) {
    ...
    for (int i = 0; i < numBuckets; i++) {
      final Bucket<Float> bucket = buckets[i];
      if(qvalue < bucket.getStart()){
        result[i] = 1;
      }else if(bucket.getStart() <= qvalue && qvalue < bucket.getEnd()){
        result[i] = (bucket.getEnd() - qvalue)/ (bucket.getEnd() - bucket.getStart());
      }else{
        result[i] = 0;
      }
    }
  ...
  private float[] allLessThan(float qvalue) {
    ...
    for (int i = 0; i < numBuckets; i++) {
      ...
      if(qvalue < bucket.getStart()){
        ...
        result[i] = (qvalue - bucket.getStart())/ (bucket.getEnd() - bucket.getStart());
      }else{
        result[i] = 1;
      }
    }
